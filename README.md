# 基于区块链带外身份索引的递归解析器身份认证原型

本仓库是方案书《基于区块链带外身份索引的递归解析器身份认证方案》的 spec-first 原型实现。

目标：在不修改 DNS 协议、DNS 报文格式或操作系统网络栈的前提下，通过应用层本地 DNS 验证封装器、链下 SQLite Indexer、本地可信缓存与链上可信锚，验证递归解析器身份后再释放 DNS 响应。

## 当前实现状态

当前主闭环已经从早期 SQLite registry prototype 升级为真实本地 Web3/Anvil 和 Docker Compose 多解析器链路：
...
- `Web3RegistryBackend`：真实读取/写入 Anvil 合约
...
- Resolver Identity Agent：`/v1/identity`、`/v1/upstreams`、`/v1/verification-chain`、`/healthz`
...
3. R1 Agent `/v1/verification-chain` 响应用对应私钥签名；
...
- `specs/001-resolver-identity-prototype/` — spec 开发文档
- `contracts/` — Solidity 合约与 Foundry 测试
- `src/resolver_identity/` — Python 原型实现
- `tests/` — 单元/集成/实验测试
- `tools/` — 本地演示与 E2E 脚本
- `docs/real_multi_resolver_testbed.md` — Docker Compose 多解析器测试床说明
