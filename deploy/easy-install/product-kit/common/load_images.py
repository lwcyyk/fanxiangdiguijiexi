...
def digest_from_reference(reference: str) -> str:
    ...
    if not match or ":latest@" in reference:
...
def inspect_digests(reference: str) -> set[str]:
    ...
    return {str(value).rsplit("@sha256:", 1)[-1] for value in values if "@sha256:" in str(value)}
...
def parse_manifest(path: Path) -> list[tuple[Path, str, str]]:
    ...
    entries = obj.get("archives")
    if not isinstance(entries, list) or not entries:
        raise ImageError("镜像清单 archives 必须是非空数组")
    parsed = []
    for entry in entries:
        ...
        reference = entry.get("image") or entry.get("reference")
...
def main() -> int:
    ...
    try:
        manifest = args.directory.resolve() / "image-archives.json"
        ...
        for archive, _, reference in entries:
            ...
            expected = digest_from_reference(reference)
